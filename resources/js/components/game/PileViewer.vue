...
<template>
    <div class="rounded-xl border border-primary/30 bg-card p-4 shadow-sm">
        ...
        <div class="flex flex-wrap gap-3">
            <button
                ...
                :disabled="disabled"
                class="flex h-20 w-14 flex-col items-center justify-center rounded-xl border-2 text-white shadow transition-all"
                :class="[
            ...
            >
                <span class="px-1 text-center text-[10px] font-bold leading-tight">{{ CLOTHING_TYPE_LABELS[card.clothing_type] }}</span>
                <span v-if="pendingCard?.id === card.id" class="mt-1 text-[8px] font-semibold opacity-90">Pick up?</span>
            </button>
        </div>
    </div>
</template>
